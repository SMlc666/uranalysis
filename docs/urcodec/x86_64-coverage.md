...
| Register move | `mov r64, imm64`, `mov r8, imm8`, `mov r/m64, r64`, `mov r64, r/m64`, `mov r/m64, imm32`, `mov m16, imm16`, `mov r/m8, r8`, `mov r8, r/m8`, `mov r/m8, imm8` | Partial | Implemented | Implemented | Yes | Not measured | Common 64-bit and byte GPR forms plus operand-size-prefixed memory immediate stores only. |
| Address calculation | `lea r64, m` | Partial | Implemented | Implemented | Yes | Not measured | Common ModRM/SIB memory only. |
| Arithmetic | `add`, `adc`, `sbb`, `sub`, `inc`, `dec`, `neg`, `mul`, `imul`, `div`, `idiv` | Partial | Implemented | Implemented | Yes | Not measured | Common 64-bit and byte GPR forms, AL imm8 forms, byte/imm8/imm32 group, `f6`/`f7`, and `ff /0`-`/1` forms only. |
| Compare and test | `cmp`, `test`, `bt`, `setcc` | Partial | Implemented | Implemented | Yes | Not measured | Common 64-bit and byte GPR forms, `cmp al, imm8`, `test al, imm8`, byte `test`, byte/imm8/imm32 group, `f6`/`f7 /0`, `bt r/m, r`, and `setcc r/m8` forms only. |
...
| SSE/AVX | `movups`, `movaps`, `xorps`, `cmpps`, `vmovdqa`, `vmovdqu`, `vmovntdq`, `vpmovmskb`, `vmulps`, `vmulsd`, `vzeroupper` | Partial | Implemented | Implemented | Yes | Not measured | Basic legacy XMM forms plus corpus-driven 2-byte and 3-byte VEX.128/VEX.256 decode for selected AVX move, multiply, mask, and cleanup forms only. |
| MMX | `movq`, `pavgb`, `pmaddwd`, `psllw`, `paddw`, `pmovmskb` | Partial | Implemented | Implemented | Yes | Not measured | Corpus-driven legacy MMX register/memory forms only; 3DNow and prefixed XMM variants remain out of scope. |
| System and privileged | `nop`, `int3`, `ud2`, `syscall`, `rdmsr`, `wrmsr` | Partial | Implemented | Implemented | Yes | Not measured | Plain NOP, selected prefixed multi-byte NOPs, `int3`, and `ud2`; privileged/system instructions remain out of first scope. |
