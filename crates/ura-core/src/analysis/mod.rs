...
use crate::{
    model::{
        AnalysisState, BasicBlock, CfgEdge, Diagnostic, Function, Instruction, StringRef,
        UserFacts, Xref,
    },
    Result,
};
...
pub fn run_initial_analysis_with_instruction_limit(
    ...
    let entry = image.entry.ok_or_else(|| {
        crate::UraError::Unsupported("binary view missing analysis entry".to_string())
    })?;
...
pub fn build_state_from_view_with_instruction_limit(
    ...
    let output =
        run_initial_analysis_with_instruction_limit(view, &user_functions, max_instructions)?;
